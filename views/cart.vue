<template>
    <div class="cart">
        ...
        <div class="cart-content">
            <div class="cart-content-main" v-for="(hero,index) in cartList">
                <div class="cart-info">
                    <img :src="allHeroList[hero.id-1].imgUrl">
                    <span>{{ allHeroList[hero.id-1].name }}</span>
                </div>
                <div class="cart-price">
                    ¥{{allHeroList[hero.id-1].price}}
                </div>
                ...
                <div class="cart-cost">¥{{allHeroList[hero.id-1].price*hero.count}}</div>
...
<script>
    ...
    export default {
        name: "cart",
        computed:{
            cartList () {//获取购物车列表(一个个对象，只有id和数量)
            ...
            costAll () {
                let cost = 0;
                this.cartList.forEach(item => {
                    cost += this.allHeroList[item.id-1].price * item.count;
                });
                return cost;
            }
        },
        ...
        methods: {
            ...
            handleCount (index, count) {
                ...
                //发送一个对象，包括要修改的id和要加减的数量，发送到mutations里面
